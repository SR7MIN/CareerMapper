...
def create_textcnn_model(embedding_dim, logdir=None):
    # 模型参数设定
    output_dim = 47
    out_channels = 66
    filter_sizes = (3, 4, 5)
    ...
    if logdir is None:
        ...
        check_points = [os.path.join(logdir, f) for f in sorted(os.listdir(logdir)) if 'tar' in f]
...
def train():
    ...
    inputs, targets, label_list, l2i_dict, i2l_dict, sentence_list = load_text_data("111.csv", 'word_vectors_train.npy',
                                                                                    False)
    rand_num = 100
    ...
    model, start, grad_vars, optimizer, raw_noise_std = create_textcnn_model(768)
...
def mix_apply(data_npy, data_csv, save_path):
    # 两份数据，一份n*20*768, 一份n*20*100
    w2v_model = gensim.models.Word2Vec.load("./words.model")
    text_list, inputs1, inputs2, i2l_dict = load_apply_data(data_npy, data_csv, w2v_model)
    inputs1 = Tensor(inputs1).to(device)
    inputs2 = Tensor(inputs2).to(device)
    model1, start, grad_vars, optimizer, raw_noise_std = create_textcnn_model(768, 'bert_cnn_logs')
    model2, start, grad_vars, optimizer, raw_noise_std = create_textcnn_model(100, 'word_cnn_logs')
    testsavedir = os.path.join(save_path, '预测结果(全).csv')
    with torch.no_grad():
        outs1 = torch.softmax(model1(inputs1[:, None]), dim=1).cpu().numpy()
        outs2 = torch.softmax(model2(inputs2[:, None]), dim=1).cpu().numpy()
        outs = (outs1 + outs2) / 2
        with open(testsavedir, 'w', newline='') as file:
            csv_writer = csv.writer(file)
...
def mix_apply_ambiguous(data_npy, data_csv, save_path):
    w2v_model = gensim.models.Word2Vec.load("./words.model")
    text_list, inputs1, inputs2, i2l_dict = load_apply_data(data_npy, data_csv, w2v_model)
    inputs1 = Tensor(inputs1).to(device)
    inputs2 = Tensor(inputs2).to(device)
...
mix_apply_ambiguous('word_vectors_apply.npy', 'all_jobs.csv', '')
